...
const APP_ID: &'static str = "farmhack";
...
pub fn happ_bundle() -> AppBundle {
    let bytes = include_bytes!("../../workdir/farmhack.happ");
    AppBundle::decode(bytes).expect("Failed to decode farmhack happ")
}
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let result: anyhow::Result<()> = tauri::async_runtime::block_on(async move {
                ...
                app.holochain()?
                    .main_window_builder(String::from("main"), false, Some(String::from("farmhack")), None).await?
                    .build()?;
...
fn signal_url() -> Url2 {
    if tauri::is_dev() {
        let internal_ip = internal_ip();
        let signal_port =
            std::option_env!("SIGNAL_PORT").expect("Environment variable SIGNAL_PORT was not set");
...
fn holochain_dir() -> PathBuf {
    if tauri::is_dev() {
        ...
        {
            let tmp_dir =
                tempdir::TempDir::new("farmhack").expect("Could not create temporary directory");
